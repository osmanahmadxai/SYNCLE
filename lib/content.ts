...
export const DESCRIPTION =
  'Open-source, self-hosted database sync. Draw a bridge from a source to one or more destinations and Syncle keeps them in sync in real time — PostgreSQL, MySQL, SQLite, MongoDB and Redis, any engine to any other. Install with one command.';
...
/** Real jobs people reach for a sync tool to do. */
export const USE_CASES: { title: string; body: string }[] = [
  {
    title: 'Migrate to a different engine',
    body: 'Backfill every row into the new database with a replay job, then leave a CDC bridge running so the two stay identical while you cut traffic over. Nothing has to be offline for it.',
  },
  {
    title: 'Feed a read replica you actually control',
    body: 'Keep a second database in step for reporting or exports without pointing analysts at production, and without paying for managed replication you cannot shape.',
  },
  {
  ...
  },
  {
    ...
    body: 'Mirror the columns a search index needs into MongoDB or a flat table, reshaped on the way across, without bolting write hooks onto the application.',
  },
  {
